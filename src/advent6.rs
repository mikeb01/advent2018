...
pub fn advent6a() -> Result<usize, Error> {
    ...
    for px in 0..grid.len() {
        ...
        flood_fill(&mut grid, (px, 0), target, GridEntry::Infinity);
    }
    ...
    for px in 0..grid.len() {
        ...
        flood_fill(&mut grid, (px, last_y), target, GridEntry::Infinity);
    }

    for py in 0..grid[0].len() {
        let target = grid[0][py];
        flood_fill(&mut grid, (0, py), target, GridEntry::Infinity);
    }
    ...
    for py in 0..grid[x_last].len() {
        ...
        flood_fill(&mut grid, (x_last, py), target, GridEntry::Infinity);
    }
...
fn flood_fill(grid: &mut [&mut [GridEntry]], node: (usize, usize), target: GridEntry, replacement: GridEntry) {
    ...
    if target == GridEntry::Infinity || target != grid[node.0][node.1] {
        return;
    }
    ...
    loop {
        ...
        if 1 <= n.0 {
            let west = (n.0 - 1, n.1);
            check_paint_queue_node(grid, west, target, replacement, &mut node_q)
        }

        if n.0 < (grid.len() - 1) {
            let east = (n.0 + 1, n.1);
            check_paint_queue_node(grid, east, target, replacement, &mut node_q)
        }

        if 1 <= n.1 {
            let north = (n.0, n.1 - 1);
            check_paint_queue_node(grid, north, target, replacement, &mut node_q)
        }

        if n.1 < (grid[n.0].len() - 1) {
            let south = (n.0, n.1 + 1);
            check_paint_queue_node(grid, south, target, replacement, &mut node_q)
        }
    }
}

fn check_paint_queue_node(
    grid: &mut [&mut [GridEntry]],
    candidate: (usize, usize),
    target: GridEntry, replacement: GridEntry,
    node_q: &mut VecDeque<(usize, usize)>) -> () {

    if grid[candidate.0][candidate.1] == target {
